...
const FIELDS: {
  ...
  {
    key: "aadhaar",
    label: "आधार · Aadhaar (local copy only)",
    placeholder: "12 digits",
    type: "text",
  },
  {
    key: "fullName",
    label: "पूरा नाम · Full name (English)",
    placeholder: "Shivam Kumar Pathak",
  },
  {
    key: "nameNative",
    label: "नाम (अपनी भाषा में) · Name in native script",
    placeholder: "शिवम कुमार पाठक",
  },
...
function ProfileDialog({
  ...
  const runFetch = () => {
    ...
    DIGILOCKER_DOCS.forEach((doc, i) => {
      timers.current.push(
        setTimeout(() => setFetchedDocs((d) => [...d, doc]), 500 + i * 550),
      );
    });
    timers.current.push(
      setTimeout(
      ...
      >
        ...
        <div className="mb-3 rounded-xl border border-indigo-200 bg-indigo-50/70 p-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-bold text-indigo-900">
              {t("DigiLocker से details लाएँ")}
            </p>
            <span className="ml-auto rounded bg-indigo-200 px-1.5 py-0.5 text-[10px] font-bold text-indigo-800">
              {t("DEMO")}
            </span>
          </div>
          ...
          {dlStep === "idle" && (
            <>
              ...
              >
                {t("🔗 Connect DigiLocker (simulated)")}
              </button>
            </>
          )}
          ...
          {dlStep === "aadhaar" && (
            <div className="mt-2 space-y-2">
              <label className="block text-xs font-semibold text-indigo-900">
                {t("आधार के आख़िरी 4 अंक · Aadhaar last 4 digits")}
              </label>
              <input
                value={aadhaarLast4}
                onChange={(e) =>
              ...
              <p className="text-center text-[10px] text-indigo-500">
                {t("Demo — कोई असली आधार नंबर न डालें, कुछ भी 4 अंक चलेंगे")}
              </p>
            </div>
          )}
          ...
          {dlStep === "otp" && (
            <div className="mt-2 space-y-2">
              <p className="text-xs text-indigo-800">
                {t("📲 OTP आपके registered mobile पर भेजा गया (simulated)")}
              </p>
              <input
                value={otp}
                onChange={(e) =>
          ...
          {(dlStep === "fetching" || dlStep === "done") && (
            <div className="mt-2 space-y-1.5">
              {fetchedDocs.map((d) => (
                <div
                  key={d}
                  className="flex items-center gap-2 rounded-lg bg-white px-2.5 py-1.5 text-xs font-medium text-stone-700"
                >
                  {t(d)}{" "}
              ...
              {dlStep === "fetching" ? (
                <p className="animate-pulse text-center text-xs font-medium text-indigo-700">
                  {t("DigiLocker से ला रही हूँ…")}
                </p>
              ) : (
                <p className="rounded-lg bg-emerald-100 px-2.5 py-1.5 text-center text-xs font-semibold text-emerald-800">
                  {t("✓ Demo पूरा हुआ — नीचे अपनी जानकारी जाँच कर Save करें")}
                </p>
              )}
            </div>
          )}
        ...
        {isSeed && dlStep === "idle" && (
          <p className="mb-2 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-800">
            {t("Sample details are prefilled. Review them before use.")}
          </p>
        )}
        <p className="mb-4 rounded-lg bg-emerald-50 px-3 py-2 text-xs leading-relaxed text-emerald-800">
          {t(
            "सेव किए बदलाव इस browser में रहते हैं। Default demo details ऐप में शामिल हैं। जनसेवक मार्गदर्शन के लिए profile details Gemini को भेजती है; आधार केवल local copy के लिए है। OTP या पासवर्ड यहाँ न रखें।",
          )}
        </p>
        ...
        >
          {t("Default details भरें · Load defaults")}
        </button>

        <div className="space-y-3">
          {FIELDS.map((f) => (
            <div key={f.key}>
              <label className="mb-0.5 block text-xs font-semibold text-stone-600">
                {t(f.label)}
              </label>
              <input
                type={f.type ?? "text"}
        ...
        >
          <h3 id="vault-title" className="font-bold text-stone-800">
            {t("🗂️ वॉल्ट · Vault")}
          </h3>
          <p className="mt-1 text-xs text-stone-600">
            {t(
              "पंजीकरण की स्थिति देखते समय इस जानकारी से मदद मिलेगी। DEMO वाला नंबर नमूना है।",
            )}
          </p>
          <label
            htmlFor="vault-registration"
            className="mt-3 block text-xs font-semibold text-stone-600"
          >
            {t("पंजीकरण संख्या · Registration number")}
          </label>
          <input
            id="vault-registration"
            value={profile.registrationNumber ?? ""}
            onChange={(e) => {
              setProfile((p) => ({ ...p, registrationNumber: e.target.value }));
              setSaved(false);
            }}
        ...
        <button
          onClick={() => {
            saveProfile(profile);
            navigator.clipboard
              .writeText(profileToShareLink(profile))
              .then(() => {
                setLinkCopied(true);
        ...
        >
          {linkCopied
            ? t("✓ Link copied!")
            : t("🔗 Share link — sample form खुलेगा, agent + details के साथ")}
        </button>
        <p className="mt-1.5 text-center text-[10px] leading-relaxed text-stone-400">
          {t(
            "Link में details सिर्फ URL के #hash में रहते हैं — server पर नहीं जाते। परिवार के किसी सदस्य को भेजिए, उनके browser में agent इन्हीं details से guide करेगा।",
          )}
        </p>
      </div>
    </div>
